...
from bz_agent.native_agent.tools.base import BaseTool, ToolResult
...
class MCPClients(ToolCollection):
    ...
    async def connect_sse(self, server_url: str,headers: dict[str, Any] | None = None, server_id: str = "") -> None:
        ...
        streams_context = sse_client(url=server_url,headers=headers)
    ...
    # ) -> None:
    #     """
    #     Connect to an MCP server using streamableHttp transport (HTTP POST + chunked JSON lines).
    #     """
    #     if not server_url:
    #         raise ValueError("Server URL is required.")
    #
    ...
    #         timeout=60.0
    #     )
    #     managed_transport = await exit_stack.enter_async_context(transport)
    #
    #     # Create ClientSession with read/write from transport
    #     session = await exit_stack.enter_async_context(
    #         ClientSession(managed_transport.read, managed_transport.write)
    #     )
    #     self.sessions[server_id] = session
    #
